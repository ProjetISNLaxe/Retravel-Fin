...
def attaqued(d,nomennemie): # fonction qui gére les combat infligé au ennemie ainsi que les recompense de fin de combat
    ...
    if nomennemie == "voleur":
        voleur.vie -= d
        if voleur.poison>0:# on gere les degat infligé par le poison
            voleur.poison-=1
            david.vie-=10
            perso_joueur.vie-=10
        if voleur.vie <= 0 :
    ...
    if nomennemie == "sinatramechante":
        if random.random() <= 0.5:# on gere l'esquive
            d=0
        sinatramechante.vie -= d
        if sinatramechante.poison==True:# on gere les degat infligé par le poison
            david.vie-=10
            perso_joueur.vie-=10
        if sinatramechante.vie <= 0 :
            tune +=200
...
def menuobjet(choix, a, c): #les actions du menu objet
    ...
    if choix == 3 and c:
        if combat.tour == 1 and mana.quantite!=0:
            mana.quantite -= 1
